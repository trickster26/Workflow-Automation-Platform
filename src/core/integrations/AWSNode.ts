...
import { S3, Lambda, SQS, SNS, SES } from 'aws-sdk';
...
export class AWSNode implements INodeType {
    ...
    switch (operation) {
      ...
      case 'getSignedUrl':
        ...
        const signedUrl = s3.getSignedUrl('getObject', {
          ...
          Expires: additionalFields.expires as number || 3600,
        });
  ...
  async handleSNSOperation(this: INodeExecuteFunctions, awsConfig: any, itemIndex: number): Promise<any> {
    ...
    switch (operation) {
      ...
      case 'listTopics':
        return await sns.listTopics().promise();
  ...
  async handleSESOperation(this: INodeExecuteFunctions, awsConfig: any, itemIndex: number): Promise<any> {
    ...
    switch (operation) {
      ...
      case 'getSendQuota':
        return await ses.getSendQuota().promise();

      case 'getSendStatistics':
        return await ses.getSendStatistics().promise();
